...
def train(model,optimizer,scheduler,criterion,train_loader,
    ...
    for e in range(epochs):
        ...
        if val_metrics['loss'] < valid_loss_min:
            ...
            torch.save(save_file, 'python/BraTs2021/UNet.pth')
        else:
...
def main(args):
    ...
    train_dataset = BraTS_Dataset(args.data_path,args.train_txt, transform=transforms.Compose([
        RandomRotFlip(),
        RandomCrop(patch_size),
        GaussianNoise(p=0.1),
        ToTensor()
    ]))
    val_dataset = BraTS_Dataset(args.data_path,args.valid_txt,transform=transforms.Compose([
        CenterCrop(patch_size),
        ToTensor()
    ]))
    test_dataset = BraTS_Dataset(args.data_path,args.test_txt,transform=transforms.Compose([
        CenterCrop(patch_size),
        ToTensor()
    ]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='python/BraTs2021/Dataset')
    parser.add_argument('--train_txt', type=str, default='python/BraTs2021/train.txt')
    parser.add_argument('--valid_txt', type=str, default='python/BraTs2021/valid.txt')
    parser.add_argument('--test_txt', type=str, default='python/BraTs2021/test.txt')
    parser.add_argument('--train_log', type=str, default='python/BraTs2021/UNet.txt')
    parser.add_argument('--weights', type=str, default='python/BraTs2021/UNet.pth')
    parser.add_argument('--save_path', type=str, default='python/BraTs2021/checkpoint/UNet')
